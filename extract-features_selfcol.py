...
df = pd.read_csv('vids_csv_straight.csv', index_col = None, header = None, sep = ',')
...
for idx,f in video_folders.iteritems():
	path = os.path.join('makaton-straight-data','makaton_vids',str(f))
	files = np.sort(np.array([os.path.splitext(filename)[0] for filename in os.listdir(path)]))
	save_path = os.path.join('makaton-straight-data','makaton_features',str(f)+'-'+'features-mobilenet')
	seq = []
	for frame in files:
